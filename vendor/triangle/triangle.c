...
#endif /* LINUX */
#ifdef TRILIBRARY
/* Vendored at vendor/triangle/ — flat layout, plain "triangle.h". */
#include "triangle.h"
#endif /* TRILIBRARY */
...
void crossProduct(double u[], double v[], double norm[])
{
...
void triInterpCentVertAttribs(vertex newVert, int i, vertex torg, vertex tdest, vertex tapex)
{
    double xx = newVert[0];
    double yy = newVert[1];
    ...
    double max = torg[i] > tdest[i] ? torg[i] : tdest[i];
    max = max > tapex[i] ? max : tapex[i];
...
(bdest == badtri->triangdest) && (bapex == badtri->triangapex)) {
    ...
    else
    {
        for (i = 2; i < 2 + m->nextras; i++)
        {
            ...
            maxAtt = max(borg[i],max(bdest[i],bapex[i]));
            minAtt = min(borg[i],min(bdest[i],bapex[i]));
            attVal = borg[i] + xi * (bdest[i] - borg[i]) + eta * (bapex[i] - borg[i]);
            newvertex[i] = min(maxAtt, max(minAtt,attVal));

            //triInterpCentVertAttribs(newvertex,i,borg,bdest,bapex);
        }
...
while (triangleloop.tri != (triangle *) NULL) {
    ...
    for (i = 2; i < 2 + m->nextras; i++)
    {
        ...
        maxAtt = max(torg[i],max(tdest[i],tapex[i]));
        minAtt = min(torg[i],min(tdest[i],tapex[i]));
        attVal = torg[i] + xi * (tdest[i] - torg[i]) + eta * (tapex[i] - torg[i]);
        palist[attribindex++] = min(maxAtt, max(minAtt,attVal));

        //    triInterpCentVertAttribs(palist,i,torg,tdest,tapex);
    }
#else /* not TRILIBRARY */
