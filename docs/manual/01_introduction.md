...
| **Example Projects** | One button per bundled example, grouped under collapsible category headers inside a scrolling list; the button's caption is the example's description |
...
A directory example may carry an `example.json` manifest with `name`,
`description` and `category` fields; the first two become the button's title and
caption. Without a manifest the folder or file name is prettified (`_` and `-`
become spaces) and the caption reads *Copy to a folder you choose — then open*.

`category` decides which collapsible header the button appears under. Examples
with no category land in **Getting Started**, which leads the list and starts
expanded; named categories follow in alphabetical order, collapsed, with their
entry count in the header. Within a group, examples keep folder-name order.

When a directory example holds more than one `.inp` and no `.oswp` — as the
SWASHES cases do, shipping a deck per solver — the alphabetically first `.inp`
is the one the button opens. The whole folder is still copied, so the other
decks are there to open from **File → Open**.
...
Two groups ship with this build (the curated list lives in `CMakeLists.txt` as
`_SWMMVIS_EXAMPLE_INPS` and `_SWMMVIS_EXAMPLE_DIRS`):

**Getting Started** — `site_drainage_model.inp`, `road_culvert.inp` and
`bellinge_2d`. These are the models the tutorials in Part V use.

**Analytical Verification (SWASHES)** — ten cases from Delestre et al. (2013),
*SWASHES: a compilation of shallow water analytic solutions for hydraulic and
environmental studies* (doi 10.1002/fld.3741), each with a closed-form solution
to check a solver against: `swashes_lake_at_rest_immersed` (§3.1.1),
`swashes_bump_subcritical` (§3.1.3), `swashes_bump_transcritical` (§3.1.4),
`swashes_bump_shock` (§3.1.5), `swashes_macdonald_short_sub2sup` (§3.2.2),
`swashes_macdonald_rain_sub` (§3.3.1), `swashes_p2d_jump_short` (§3.5.4),
`swashes_stoker_wet_dam_break` (§4.1.1), `swashes_ritter_dry_dam_break` (§4.1.2)
and `swashes_thacker_radial_2d` (§4.2.2).

Each SWASHES folder holds the decks its case supports — `1d_dynwave.inp`
(dynamic-wave routing, the default open target), `1d_fv.inp` (finite-volume
routing) and `2d_explicit.inp` (2D shallow-water mesh) — plus `reference.csv`
with the analytic depth and unit discharge, and a `README.md` giving the case
parameters and boundary conditions. Input files only: no results ship. Three
cases are single-dimension by nature — the rain and pseudo-2D cases are 1D
only, and the radial Thacker case is 2D only. The decks are generated by the
SWASHES suite in the openswmm.engine benchmarks and imported by
`scripts/import_swashes_examples.py`; re-run that script to refresh them.
