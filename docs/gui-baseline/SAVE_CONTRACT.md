...
The multiple-mesh gate remains open: the current Save loop synchronizes every candidate layer to a single engine and marks each pushed layer saved, while only the chosen external mesh is restored/patched. The `.oswp` stores display configuration, not mesh edit payloads. Count equality is not mesh identity. W1 must establish explicit ownership for inactive dirty layers and same-count replacement topology before claiming this gate passes; selecting the active file alone (Phase 08) did not resolve it. Engine APIs can also accept invalid values (for example non-finite elevations), and count-mismatch fallbacks can skip fields; Phase 09's rejection checks do not certify complete validation or persistence in those paths.
