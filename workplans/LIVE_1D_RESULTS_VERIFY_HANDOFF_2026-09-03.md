...
**Step 1 — Engine: PASS.** Build clean; targeted ctest 16/16 green including
the new `test_engine_output_reader_live` (all four `live_out` fixtures
written). Installed + codesigned. Flush timing on
`375_H&H_Elements.inp` (Debug, quiet machine, interleaved): parent
15.51/15.54 s vs flush-commit 15.68/16.24 s → ≤ noise; recorded in
`tests/artifacts/live_1d/flush_timing.csv`. Note: the parent worktree
(6eee5d65, clean checkout) does **not compile standalone** —
`SurfaceRouter2D.cpp` references `SurfaceTransportState` fields that exist
only in the S4 session's uncommitted hunks; A/B built with
`OPENSWMM_BUILD_2D=OFF` (flush change is 1D-side).

**Step 2 — GUI: PASS** after two blockers unrelated to this feature:
(1) `tests/unit/test_numberformat.cpp` did not compile at HEAD — the newer
axis presets (Scientific2/3/4, Engineering2/3, ThousandsInteger, from
f8db143) were never added to its using-declarations; fixed in gui 66a5e27.
(2) A live-peer race: `SWMM_ThreadInfo` landed in the engine header mid-round
after the engine install, so `simulationoptionsdialog.h` failed against the
stale installed header — engine rebuilt/reinstalled, then clean. A disk-full
stall (≈1 GB free) interrupted one build; resumed after cleanup.
ctest `profilebuilder|comparisonplot`: 11/11 green, including the new
`AppendPeriods_MatchesFullComputeExactly` / `_RejectsShrunkOrForeignInput`.
No test target matches `resultslayer` or `preferences` in this tree — those
regex clauses matched nothing (no such suites exist to run).

**Steps 3–4 — Smoke + parity: MANUAL, not yet done.** `build/SWMMVis.app`
launched for interactive checks. Companion
`2D_CONTEXT_MENU_RAINFALL_VERIFY_HANDOFF_2026-09-02.md` remains unverified
(no Results section); its code compiles as part of this tree's build.
